...
public class GeoHashBoundingBoxQuery implements GeoHashQuery, Serializable {
    ...
    public GeoHashBoundingBoxQuery(BoundingBox bbox, boolean characterCompatible) {
        int fittingBits;
        if (characterCompatible) {
            fittingBits = GeoHashSizeTable.numberOfBitsForOverlappingGeoHashCharacterCompatible(bbox);
        } else {
            fittingBits = GeoHashSizeTable.numberOfBitsForOverlappingGeoHash(bbox);
        }
        WGS84Point center = bbox.getCenterPoint();
        GeoHash centerHash = GeoHash.withBitPrecision(center.getLatitude(), center.getLongitude(), fittingBits);
